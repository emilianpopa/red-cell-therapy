...
export default function ProductSection({ data }: ProductSectionProps) {
  const manualStandImage = buildImageUrl(data.manualStandImage, 600, 800)
  const motorizedStandImage = buildImageUrl(data.motorizedStandImage, 600, 800)
    ...
    <section className="py-20 px-4 md:px-8 bg-white">
      <div className="max-w-6xl mx-auto">
        ...
        <h2 className="text-4xl md:text-5xl text-[#484F5D] text-center mb-6">
          {data.sectionTitle || 'One Product - Done Properly'}
        </h2>
          ...
          <div className="bg-[#EFEEEA] rounded-xl p-8 shadow-md mb-8">
            ...
            <div className="space-y-4">
              {data.specifications.map((spec) => (
                <div key={spec._key} className="grid grid-cols-1 md:grid-cols-3 gap-4 pb-4 border-b border-[#484F5D]/20 last:border-0">
                  <div className="font-semibold text-[#484F5D]">{spec.label}</div>
                  <div className="md:col-span-2 text-[#484F5D]/80">{spec.value}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* CTA Link */}
        {data.specsDocumentUrl && (
          <div className="text-center">
            <a
              href={data.specsDocumentUrl}
